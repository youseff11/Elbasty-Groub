...
class ProductVariantInline(nested_admin.NestedStackedInline):
    ...
    def image_preview(self, obj):
        if obj.variant_image:
            return format_html(
                '<div class="variant-main-img-wrap" data-variant-id="{}" style="display:inline-block; position:relative; background:#ffffff; padding:5px; border-radius:8px; border: 1px solid #eee;">'
                '<img src="{}" '
                'data-img-id="{}" '
                'data-img-url="{}" '
                ...
                'style="width: 100px; height: 100px; object-fit: contain; cursor: grab; transition: border 0.2s;" />'
                '</div>',
